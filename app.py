...
# --- CACHE INTELIGENTE (MOTOR GOOGLE) ---
@st.cache_data(ttl=600, show_spinner="Sincronizando...")
...
if dados_ok:
    ...
    if st.sidebar.button("Forçar Recarga"): st.cache_data.clear(); st.rerun()

# Upload Histórico (Mantido como Backup Manual)
st.sidebar.markdown("---")
uploaded_hist = st.sidebar.file_uploader("Importar Excel (Opcional)", type=["xlsx"], key="hist", accept_multiple_files=True)
if uploaded_hist:
    dfs = []
    try:
        ...
        if dfs:
            ...
            st.sidebar.success(f"Importado: {len(novos)}")
    except: pass
